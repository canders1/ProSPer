...
class KneserNeyBaseNGram(NGram):
    def __init__(self, sents, n, corpus='', D=0.1):
        """
        sents -- list of sents
        n -- order of the model
        D -- Discount value
    ...
    def N_dot_dot(self):
        """
        Returns the sum of N_dot_tokens(w) for all w in the vocabulary
        """
...
class KneserNeyNGram(KneserNeyBaseNGram):
    ...
    def cond_prob(self, token, prev_tokens=tuple()):
        ...
        # lowest ngram
        if not prev_tokens and n > 1:
            aux1 = float(len(self.N_dot_tokens((token,))))
            aux2 = self.N_dot_dot()
            # addone smoothing
        ...
        if len(prev_tokens) == n-1:
            c = float(self.count(prev_tokens)) + 1
